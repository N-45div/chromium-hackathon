...
const getHealthFactorHandler = async (req, res) => {
    console.log('Received request for /health-factor', req.query);
    if (!agent) {
        res.status(503).json({ error: 'Agent not initialized yet.' });
        return;
    }
    ...
    try {
        ...
        console.log('Health factor fetched:', healthFactor);
        res.status(200).json({ healthFactor });
    } catch (error) {
        console.error('Error in health-factor:', error);
