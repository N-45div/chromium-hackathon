...
async function main() {
    ...
    const functionsTriggerFactory = new ethers.ContractFactory(FunctionsTriggerArtifact.abi, FunctionsTriggerArtifact.bytecode, wallet);
    // Deploy the contract with all required arguments
    const functionsTrigger = await functionsTriggerFactory.deploy(functionsRouterAddress, // router
    donIdBytes, // donId
