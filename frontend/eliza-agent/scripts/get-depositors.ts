...
async function main() {
  ...
  for (let i = 0; i <= latestBlock; i += chunkSize) {
    ...
    const toBlock = Math.min(i + chunkSize - 1, latestBlock);
      ...
      console.log(`  - Scanned blocks ${fromBlock} to ${toBlock}: Failed. Error: ${(error as Error).message}`);
    }
  }
  ...
  if (depositors.length === 0) {
    ...
    depositors.forEach((depositor: string, index: number) => {
