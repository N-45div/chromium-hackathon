  ...
  const fetchAvailable = useCallback(async () => {
    ...
    try {
      ...
      if (chainId !== CHAIN_IDS.SEPOLIA) {
        toast({ title: "Warning", description: `Please switch to Sepolia (Chain ID: ${CHAIN_IDS.SEPOLIA})`, variant: "default" })
      }

      // Check BorrowManagement.sol on Fuji
      let collateralToken = "None"
      let borrowToken = BORROW_USDC
      try {
        ...
        collateralToken = TOKEN_SYMBOLS[balanceFields.collateralToken] || "None"
        borrowToken = balanceFields.borrowToken !== ethers.ZeroAddress ? TOKEN_SYMBOLS[balanceFields.borrowToken] || "Unknown" : "USDC"
        if (balanceFields.borrowToken === ethers.ZeroAddress) {
