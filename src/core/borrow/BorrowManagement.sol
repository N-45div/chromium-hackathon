...
import {CCIPHelper} from "src/core/CCIPHelper.sol";
...
contract BorrowManagement is IBorrowManagement, CCIPReceiver, Ownable {
    ...
    /// @dev Internal wrapper around CCIPHelper.safeSend using the contract's router.
    function _safeCCIPSend(uint64 destChain, Client.EVM2AnyMessage memory msgData) internal returns (bytes32) {
        return CCIPHelper.safeSend(destChain, msgData, getRouter());
    }
    ...
    function borrowApply(uint256 amount, bytes32 commitmentHash, bytes calldata proof) external {
        ...
        // nullifierHash generated by privacy circuit
        ...
        // --- CCIP SEND (privacy) ---
        uint64 destChainSelectorP = uint64(availableBorrowTokenBalance[msg.sender].sourceChainId);
        bytes memory receiverP = abi.encode(address(0));
        bytes memory dataP = abi.encode(crossChainBorrowInfo);
        Client.EVMTokenAmount[] memory tokenAmountsP = new Client.EVMTokenAmount[](0);
        Client.EVM2AnyMessage memory messageP = Client.EVM2AnyMessage({
            receiver: receiverP,
            data: dataP,
            tokenAmounts: tokenAmountsP,
            extraArgs: bytes(""),
            feeToken: address(0)
        });
        bytes32 msgIdP = _safeCCIPSend(destChainSelectorP, messageP);
        emit BorrowApplyMessageSent(msgIdP, destChainSelectorP);
    ...
    function repay(uint256 amount) external {
        ...
        CCIPHelper.safeSend(destChainSelector, message, getRouter());
